...
class LsMembersDetailsSpider(scrapy.Spider):
    ...
    def parse_profile(self,response):
        ...
        first_table = [_.strip() for _ in response.css("table#ContentPlaceHolder1_Datagrid1").css("td.griditem2::text").extract() if len(_.strip())>0]
        ...
        for i in range(len(second_table_values)):
            ...
            if heading == 'Date of Birth':
                ...
                item["phone"] = re.findall(' [0-9]{11} ',value)
        # yield item
        self.collection.insert_one(item)
        # query = {"email":item["email"], "phone":item["phone"]}
        # print(query)

        # self.collection.update_one({"_id":item["_id"]},{"$set":query})
        # print(item["_id"]," inserted")
